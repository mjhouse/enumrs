...
mod tests {
    ...
    fn test_name_attribute() {
		...
		assert_eq!(Country::UMI.name(),Some("United States Minor Outlying Islands"));
    }

    #[test]
    fn test_uuid_attribute() {
		assert_eq!(Country::PSE.uuid(),Some("c1efc87a-ea2b-4aa4-8f1b-405245c83a42"));
		assert_eq!(Country::UMI.uuid(),Some("64d78320-495e-4602-83e9-c07e6ba55de5"));
    }

    #[test]
    fn test_description_attribute() {
		assert_eq!(Country::PSE.description(),Some("Palestine, State of (PSE)"));
		assert_eq!(Country::UMI.description(),Some("United States Minor Outlying Islands (UMI)"));
    }
}